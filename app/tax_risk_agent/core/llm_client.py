...
class LLMClient(Protocol):
  ...
  任何类只要实现 summarize_findings方法,就可以作为LLMCline 使用
  """
...
class offlineLLMClient:
  ...
  def summarize_findings(self, findings:list[RiskFinding]) -> str:
    ...
    tittles = "、".join(finding.title for finding in findings)
    return f"本轮诊断形成{len(findings)}重点关注{tittles}。"

class LangChainLLMClient:
  """通过 LangChain 调用OpenAI-compatible Qwen服务"""
  ...
  def __init__(self, base_url:str, api_key:str, model:str):
    ...
    self.chat_model = ChatOpenAI(
      base_url=base_url,
      api_key=api_key,
      temperature=0.2,  # 控制大模型输出“随机性/发散程度”的参数。尽量让模型输出稳定、确定、保守。相同输入下，模型更倾向于每次给出类似甚至相同的结果
    )
  ...
  def summarize_findings(self, findings: list[RiskFinding]) -> str:
    ...
    response = self.chat_model.invoke(
      [
        (
          "system",
          "你是一个税务风险稽查专家,请基于结构化风险结论生成一段谨慎、可审计的中文摘要。",
        ),
...
def build_llm_client(provider: str, base_url: str, api_key: str, model: str) -> LLMClient:
  ...
  provider=openai_compatible 时优先接入 Qwen;初始化失败时间降级为离线摘要器
  """
